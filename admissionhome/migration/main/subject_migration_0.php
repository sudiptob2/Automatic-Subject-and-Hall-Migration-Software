...
try{ 
	...
	for($i=1;$i<=7;$i++)
	{
		if($subject[$row['choice_'.$i]]!=0 && $row['selected_subject']==NULL)
		{
			$stmt= $conn->prepare("update posted_data set selected_subject='".$row['choice_'.$i]."',flag=1 where MERIT=$d and UNIT='A'");
		    $stmt->execute();
